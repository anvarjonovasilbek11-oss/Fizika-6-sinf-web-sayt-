...
const TextbookSelection = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-120px)] space-y-12 pb-20">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-4 bg-white/5 backdrop-blur-xl border border-white/10 px-6 py-3 rounded-2xl">
          ...
          <span className="text-xs font-black uppercase text-slate-300 tracking-widest">7 ta Bob jami mahsulot</span>
        </div>
      </div>
              ...
              <motion.button
                ...
                className={`
                  ...
                  ${selectedChapter?.id === chapter.id 
                    ? 'bg-gradient-to-r from-neon-purple/20 to-transparent border-neon-purple shadow-[0_0_30px_rgba(188,19,254,0.1)]' 
                    ...
                    >
                      ...
                      <div className="p-8 space-y-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Darslik №{lesson.id}</span>
                        </div>
                        
                        <h3 className="text-xl font-bold text-white group-hover:text-neon-purple transition-colors line-clamp-2">
                          {t(`lesson_${lesson.id}`) !== `lesson_${lesson.id}` ? t(`lesson_${lesson.id}`) : lesson.title}
                        </h3>
                        
                        <div className="pt-4 flex items-center gap-2 text-xs font-black uppercase text-slate-500 tracking-[0.2em] group-hover:text-white transition-colors">
